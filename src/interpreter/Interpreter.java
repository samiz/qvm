...
public class Interpreter
{
	...
	public void RunCode(String program) throws ReaderException, VmException, InterpreterException
	{
		AST tree = Reader.read(program, "<untitled>");
		decodeInstructions(tree, vm);
		vm.pushMainMethod();
		vm.halted = false;
		vm.run();
	}
	public void initStdLib()
	{
		...
		vm.registerMethod("-", 
				new Class[]{Double.class, Double.class},
				new SubtractDoubleDouble());
		vm.registerMethod("print", 
	...
	private void decodeInstructions(AST tree, VM avm) throws NonDecodableInstructionException
	{
		if(tree.matchSymAt(0, "method"))
		{
			String mname = tree.symbolAt(1);
			Method m = decodeMethod(mname,
						 tree.intAt(2),
						 tree.VectorAt(3),
						 avm);
	...
	private Method decodeMethod(String name, int arity, VectorAst instructions, VM avm) throws NonDecodableInstructionException
	{
	...
	private Instruction decodeInstruction(AST i, VM avm) throws NonDecodableInstructionException
	{
		...
		{
			return new Dispatch(i.symbolAt(1),
								i.intAt(2)).meta(i);
		}
